...
def get_menu_user(user: Usuario) -> List[Dict]:
    ...
    Paramters: user (Usuario): User owner of menu
    Returns: List(Dict): All menus for the user
    """
    ...
    def get_parents(menu_id):
        if menu_id:
            if not list(filter(lambda x: x['id'] == menu_id, user_menu)):
                menu = Menu.objects.values('id', 'menu', 'orden', 'parent_id').get(id=menu_id)
    ...
    for item in menus:
        menu = Menu.objects.values('id', 'menu', 'orden', 'parent_id').get(id=item)
        user_menu.append(menu)
        get_parents(menu['parent_id'])
